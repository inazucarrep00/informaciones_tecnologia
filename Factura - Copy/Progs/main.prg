...
FUNCTION VigenciaContrasena()
    ...
    DO CASE 
      CASE BETWEEN(DiasTranscurridosClave,1,5)
  			=MESSAGEBOX("Su contraseña esta proxima a vencer, "+;
              		   "el acceso al SISTEMA sera negado",48,"Advertencia")
             		   
       CASE DiasTranscurridosClave=0       
       		=MESSAGEBOX("Su contraseña vence el dia de HOY..."+CHR(13)+;
             		      "el acceso al SISTEMA sera negado",48,"Advertencia")
             		      
       CASE DiasTranscurridosClave<0       
       		=MESSAGEBOX("Su contraseña ha vencido ya no tiene acceso al SISTEMA",48,"Advertencia")
            RETURN .F.    
       OTHERWISE 
            RETURN .T.        
